...
function findUser(username) {
  ...
  console.log(`[Web] Buscando usuário: '${sanitizarParaLog(normalized)}' dentro das chaves: [${Object.keys(users).join(", ")}]`);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(`[Web] URL inválida ou malformada recebida de ${ip}: ${sanitizarParaLog(req.url)}`);
      ...
      if (req.method === 'POST' && pathname === '/login') {
        try {
          ...
          if (!user) {
            console.warn(`[Web] Login falhou: Usuário '${sanitizarParaLog(username)}' não encontrado (IP: ${ip})`);
          } else {
            ...
            if (isPasswordValid && isActive) {
              ...
              console.log(`[Web] Login bem-sucedido: ${sanitizarParaLog(username)} (IP: ${ip})`);
              return sendJson(res, 200, { ok: true });
            } else if (!isPasswordValid) {
              console.warn(`[Web] Login falhou: Senha incorreta para o usuário '${sanitizarParaLog(username)}' (IP: ${ip}). Verifique o hash no log de Auth.`);
            } else {
              console.warn(`[Web] Login falhou: Usuário '${sanitizarParaLog(username)}' está com status inativo (${user.status}) (IP: ${ip})`);
            }
          }
      ...
      if (pathname !== '/login' && pathname !== '/register' && !isAuthenticated(req)) {
        ...
        console.warn(`[Web] 401 Acesso negado para ${sanitizarParaLog(pathname)} | IP: ${ip}`);
      ...
      // Rota não encontrada
      console.warn(`[Web] 404 Not Found: ${req.method} ${sanitizarParaLog(pathname)}`);
      res.writeHead(404, { 'Content-Type': 'text/plain' });
      res.end('Not found');
    } catch (globalErr) {
      console.error(`[ALERTA:web] 500 Internal Server Error em ${sanitizarParaLog(req.url)}:`, globalErr);
      if (!res.headersSent) {
